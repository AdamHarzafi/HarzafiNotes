...
// ── Attesa Firebase (Nuovo Database Harzafi Notes) ────────────
...
function entraNelPortale(nomeUtente) {
    ...
    window.location.href = 'dashboard.html'; // Porta alla dashboard di Notes
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // MANTIENI STESSA STRUTTURA: Cerca in "studenti" o "docenti"
    window.populateUserDropdown = function (role) {
        ...
        if (typeof window.db !== 'undefined') {
            window.db.collection(collectionName).orderBy("nome", "asc").get()
                ...
                .catch(() => { optionsContainer.innerHTML = '<div class="custom-option" style="color:var(--danger);">Errore di connessione</div>'; });
        }
    ...
    document.getElementById('login-form').addEventListener('submit', async e => {
        ...
        if (!uName || !selectedUserEmail)       { errorMsg.innerText = "Seleziona un utente."; errorMsg.style.display = 'block'; return; }
        if (!pass)                              { errorMsg.innerText = "Inserisci la password."; errorMsg.style.display = 'block'; return; }
        ...
        if (isVpn) {
            errorMsg.innerText  = "Disattiva la VPN."; errorMsg.style.display = 'block';
            submitBtn.innerText = "ENTRA"; submitBtn.disabled  = false; return;
        }
        ...
        if (window.globalTurnstileToken) { window.eseguiAccessoServer(); } 
            ...
            setTimeout(() => {
                if (window.isWaitingForToken && !window.turnstileCallbackFired) {
                    window.isWaitingForToken = false; window.eseguiAccessoServer();
                }
            }, 2500);
        }
    });
    ...
    googleBtn.addEventListener('click', async () => {
        ...
        googleBtn.innerHTML  = `<div style="font-weight:800; color:#374151;">CARICAMENTO...</div>`;
        ...
        window.auth.signInWithPopup(provider).then(async result => {
                ...
                if (email.endsWith("@" + targetDomain)) {
                    ...
                    window.auth.signOut().then(() => {
                        googleErrorMsg.innerText = `Usa email @${targetDomain}`; googleErrorMsg.style.display = 'block';
                        googleBtn.innerHTML = originalHTML; googleBtn.disabled  = false;
                    });
                }
            }).catch(() => {
                googleErrorMsg.innerText = "Accesso annullato."; googleErrorMsg.style.display = 'block';
                googleBtn.innerHTML = originalHTML; googleBtn.disabled  = false;
            });
    });
        ...
        if (typeof window.db !== 'undefined') {
            window.db.collection("studenti").where("HID", "==", inputVal).get()
                .then(async snap => {
                    ...
                    } else { throw new Error("HID Errato"); }
                }).catch(() => {
                    hidErrorEl.innerText = "HID non valido."; hidErrorEl.style.display = 'block';
    ...
    document.getElementById('btn-forgot-pass').addEventListener('click', e => {
        e.preventDefault();
        otpStep1.style.display = 'block'; otpStep3.style.display = 'none'; otpEmailInput.value = '';
    ...
    document.getElementById('btn-send-otp').addEventListener('click', async function () {
        ...
        if (!emailVal.includes('@')) { errorDiv.innerText = "Email non valida."; errorDiv.style.display = 'block'; return; }
        
        errorDiv.style.display = 'none'; this.innerText = 'Invio...'; this.disabled = true;

        try {
            const snap = await window.db.collection(targetCollectionOTP).where('email', '==', emailVal).get();
            if (snap.empty) throw new Error("Email non trovata.");
            await window.auth.sendPasswordResetEmail(emailVal);
            otpStep1.style.display = 'none'; otpStep3.style.display = 'block';
        } catch (err) {
            errorDiv.innerText = "Errore. Riprova."; errorDiv.style.display = 'block';
        } finally {
            this.innerText = 'INVIA LINK'; this.disabled = false;
        }
    });
});